---
title: "ATAC-Seq: RE HOMER overlap diff acc peaks and diff expr genes"
...
files <- list.files(path = "input/motif_homer", pattern = "knownResults.txt$", full.names = T, recursive = T)
files <- files[grep(pattern = "E_C|E_L", x = files, invert = T)]

names(files) <- gsub(pattern = ".*ive/|\\/.*", replacement = "", x = files)

...
date: "<b>Created on:</b> 2020-11-04 <br> <b>Updated on:</b> `r Sys.Date()`"
output:
...
c <- ggplot(data = arrange(go_df, Hyper_Fold_Enrichment), mapping = aes(
  ...
  facet_wrap(~Group, scales = "free_y", labeller = label_wrap_gen(width = 22), nrow = 2) +
  # theme_bw(base_family = "Helvetica") +
  th +
  guides(
...
cg <- gridExtra::grid.arrange(egg::set_panel_size(p = c, width = unit(4, "cm"), height = unit(5, "cm")))
...
dg <- gridExtra::grid.arrange(egg::set_panel_size(p = d, width = unit(4, "cm"), height = unit(5, "cm")))
...
go_df$name <- plgINS:::breakStrings(x = as.character(go_df$name), minSizeForBreak = 43)
...
c1 <- ggplot(data = arrange(go_df, Hyper_Fold_Enrichment), mapping = aes(
  ...
  labs(
    color = expression("adjusted" ~ italic(P) ~ " "),
    size = "Annotated genes"
  ) + theme(axis.text.y = element_text(lineheight = 0.65), legend.spacing = unit(1, "cm"))

cg1 <- gridExtra::grid.arrange(egg::set_panel_size(p = c1, width = unit(3.25, "cm"), height = unit(7.5, "cm")))
...
go_df_s$name <- plgINS:::breakStrings(x = as.character(go_df_s$name), minSizeForBreak = 43)
...
dg1 <- gridExtra::grid.arrange(egg::set_panel_size(p = d1, width = unit(3.25, "cm"), height = unit(7, "cm")))

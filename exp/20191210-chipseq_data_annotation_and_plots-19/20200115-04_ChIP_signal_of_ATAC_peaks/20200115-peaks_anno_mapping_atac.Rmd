...
res <- data.frame(rowData(data))[, -c(2:6)]
inc <- data.frame(peakAnno@anno)

tab <- merge(inc, res, by = "Name", all.y = TRUE)
writexl::write_xlsx(x = tab, path = "./output/atac_diff_chromatin_accessibility.xlsx", col_names = T, format_headers = T)

tab.sig <- tab[tab$qvalue <= 0.05, ]
writexl::write_xlsx(

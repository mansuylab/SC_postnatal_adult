...
if (!file.exists("./output/peaks_anno_plots.RData")) {
  ...
  tagMatrix.1kb <- mclapply(all, function(x) getTagMatrix(x, windows = promoter),
    mc.preschedule = F, mc.cores = length(all)
  )

  # Tag matrix 5kb promoters
  tagMatrix.5kb <- mclapply(all, function(x) getTagMatrix(x, windows = regions),
    mc.preschedule = F, mc.cores = length(all)
  )
...
# Average Profile
```{r 20191118-peaks-annotation-other-figs-10, fig.align='center', fig.width=11, fig.height=8.5, warning=FALSE, message=FALSE}

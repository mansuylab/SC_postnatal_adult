...
if (!file.exists(nm_rna_counts)) {
  rna_f <- list.files("input/rnaseq", pattern = "\\.bw$", full.names = TRUE, recursive = T)
  names(rna_f) <- gsub(pattern = "\\.bw", replacement = "", x = basename(rna_f))

  # rna_f <- BigWigFileList(rna_f)

  counts_rna <- mylapply(rna_f, function(x) {
    coverage <- import(x, as = "RleList")
    c <- mylapply(coverage, function(x) data.frame(sum(Views(x, ranges(tss_5k)))))
    d <- rlist::list.cbind(c)
    d <- d[names(tss_5k), ]
    counts <- data.frame(counts = rowSums(d))
    return(counts)
  })

  mat_RNA_counts <- rlist::list.cbind(counts_rna)
  colnames(mat_RNA_counts) <- names(counts_rna)
  save(mat_RNA_counts, file = nm_rna_counts)
} else {
  load(nm_rna_counts)
}
```
...
create_heatmap <- function(nc = 4) {
  ...
  if (!file.exists(pdf_file)) {
    ...
    hm_atac <- EnrichedHeatmap(mat_AS$Adult_NF,
      ...
      axis_name = axis_name,
      top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = 2:(nc + 1)), ylim = c(0, 55)))
    ) +
    ...
    hm_bs <- EnrichedHeatmap(mat_BS$PND7_SRX749887,
      ...
      axis_name = axis_name,
      top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = 2:(nc + 1)), ylim = c(0, 3)))
    ) +
    ...
    hm_rna_counts <- Heatmap(data.matrix(log1p(mat_RNA_counts[, 1:3])),
      name = "RNA Expression merged",
      show_row_names = FALSE,
      show_column_names = FALSE
    ) +
      Heatmap(data.matrix(log1p(mat_RNA_counts[, 4:ncol(mat_RNA_counts)])),
        name = "RNA Expression",
        show_row_names = FALSE,
        show_column_names = FALSE
      )
    ...
    if (nc == 1) {
      ht_list <- hm_atac + hm_chip + hm_bs + hm_rrbs + hm_rna + hm_rna_counts
      draw(ht_list, ht_gap = unit(2, "mm"))
      hm_rna_counts
    } else {
      ht_list <- hm_partition + hm_atac + hm_chip + hm_bs + hm_rrbs + hm_rna + hm_rna_counts
      draw(ht_list,
...
bsselectR::as_iframe(widget = bs, file = "qval_05.html")
```

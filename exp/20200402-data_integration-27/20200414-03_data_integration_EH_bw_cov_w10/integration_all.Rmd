...
if (!file.exists(nm_atac)) {
  ...
  atac_bw <- mylapply(atac_files, function(x) rtracklayer::import(x), mc.cores = length(atac_files))

  mat_AS <- mylapply(atac_bw, FUN = function(x) {
    normalizeToMatrix(x, tss_mm10,
      extend = 5000,
      w = 10, value_column = "score",
      include_target = TRUE
    )
...
if (!file.exists(nm_chip)) {
  ...
  chip_bw <- mylapply(chip_files, function(x) rtracklayer::import(x), mc.cores = length(chip_files))
...
if (!file.exists(nm_bs)) {
  bs_files <- list.files("input/bsseq", pattern = "\\.bw$", full.names = TRUE)
  names(bs_files) <- gsub(pattern = "\\.bw", replacement = "", x = basename(bs_files))
  bs_bw <- mylapply(bs_files, function(x) rtracklayer::import(x), mc.cores = length(bs_files))

  mat_BS[3:4] <- mylapply(bs_bw[3:4], FUN = function(x) {
    normalizeToMatrix(x, tss_mm10,
      extend = 5000,
      w = 10, value_column = "score",
      include_target = TRUE
...
if (!file.exists(nm_rrbs)) {
  rrbs_files <- list.files("input/rrbs", pattern = "\\.bw$", full.names = TRUE)
  names(rrbs_files) <- gsub(pattern = "\\.bw", replacement = "", x = basename(rrbs_files))
  rrbs_bw <- mylapply(rrbs_files, function(x) rtracklayer::import(x), mc.cores = length(rrbs_files))

  mat_RRBS <- mylapply(rrbs_bw, FUN = function(x) {
    normalizeToMatrix(x, tss_mm10,
      extend = 5000,
      w = 10, value_column = "score",
      include_target = TRUE
...
if (!file.exists(nm_rna)) {
  ...
  rna_bw <- mylapply(rna_files, function(x) rtracklayer::import(x), mc.cores = length(rna_files))
...
create_heatmap <- function(nc = 4) {
  ...
  if (!file.exists(pdf_file)) {
    ...
    # col_atac <- colorRamp2(c(0, 2.5, 5), c("blue", "white", "red"))
    ...
    hm_bs <- EnrichedHeatmap(log1p(mat_BS$PND7_SRX749887),
      name = "PND7 BS",
      # col = c("white", "#ba1168"),
      column_title = "PND7 BS",
      top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = 2:(nc + 1))))
    ) +
      EnrichedHeatmap(log1p(mat_BS$PND14_SRX749892),
        name = "PND14 BS",
        # col = c("white", "#ba1168"),
        column_title = "PND14 BS",
        top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = 2:(nc + 1))))
      ) +
      EnrichedHeatmap(log1p(mat_BS$PNW8_SRX688595),
        name = "PNW8 BS 1",
        # col = c("white", "#ba1168"),
        column_title = "PNW8 BS 1",
        top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = 2:(nc + 1))))
      ) +
      EnrichedHeatmap(log1p(mat_BS$PNW8_SRX688596),
        name = "PNW8 BS 2",
        # col = c("white", "#ba1168"),
    ...
    hm_rrbs <- EnrichedHeatmap(log1p(mat_RRBS$PND8),
      name = "PND8 RRBS",
      # col = c("white", "#ba1168"),

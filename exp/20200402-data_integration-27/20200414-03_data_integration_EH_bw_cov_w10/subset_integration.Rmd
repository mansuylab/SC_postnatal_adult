...
if (!file.exists(nm_bs)) {
  ...
  bs_bw <- mylapply(bs_files, function(x) rtracklayer::import(x, selection = tss_5k))

  mat_BS <- mylapply(bs_bw, FUN = function(x) {
    normalizeToMatrix(x, tss_mm10,
      extend = 5000,
      w = 10, value_column = "score",
      include_target = TRUE
...
if (!file.exists(nm_rrbs)) {
  ...
  rrbs_bw <- mylapply(rrbs_files, function(x) rtracklayer::import(x, selection = tss_5k))
...
bsselectR::as_iframe(widget = bs, file = "v2.html")
```

...
lab <- salmon
lab@gene.counts <- cpm(calcNormFactors(DGEList(lab@gene.counts + 1)), log = TRUE)

load("input/SC_controls_rnaseq_lit_salmon.tds.RData")
lit <- salmon
lit@gene.counts <- cpm(calcNormFactors(DGEList(lit@gene.counts + 1)), log = TRUE)

load("input/SC_controls_rnaseq_lab_june2021.tds.RData")
ns <- salmon
ns@gene.counts <- cpm(calcNormFactors(DGEList(ns@gene.counts + 1)), log = TRUE)


df1 <- data.frame(lab@gene.counts)
df1 <- data.frame(Genes = rownames(df1), 
                  PND8 = rowMeans(df1[, grep(pattern = "PND8", colnames(df1))]),
                  PND15 = rowMeans(df1[, grep(pattern = "PND15", colnames(df1))]),
                  PNW21 = rowMeans(df1[, grep(pattern = "Adult", colnames(df1))])
                  )
df1 <- melt(df1)
colnames(df1)[2:3] <- c("Group", "Lab")

df2 <- data.frame(lit@gene.counts)
df2 <- data.frame(Genes = rownames(df2), 
                  PND8 = rowMeans(df2[, grep(pattern = "PND7", colnames(df2))]),
                  PND15 = df2[, grep(pattern = "PND14", colnames(df2))],
                  PNW21 = df2[, grep(pattern = "PNW8", colnames(df2))]
                  )
df2 <- melt(df2)
colnames(df2)[2:3] <- c("Group", "Literature")


df3 <- data.frame(ns@gene.counts)
df3 <- data.frame(Genes = rownames(df3), 
                  PND8 = rowMeans(df3[, grep(pattern = "PND8", colnames(df3))]),
                  PND15 = rowMeans(df3[, grep(pattern = "PND15", colnames(df3))]),
                  PNW21 = rowMeans(df3[, grep(pattern = "PNW21", colnames(df3))])
                  )
df3 <- melt(df3)
colnames(df3)[2:3] <- c("Group", "Lab (new)")


df <- Reduce(merge, list(df1, df2, df3))
df$Group <- gsub(pattern = "PND8", replacement = "PND7/PND8", x = df$Group)
df$Group <- gsub(pattern = "PND15", replacement = "PND14/PND15", x = df$Group)
df$Group <- gsub(pattern = "PNW21", replacement = "PNW8/PNW21", x = df$Group)

df$Group <- factor(df$Group, unique(df$Group)[c(2,1,3)])

# df <- head(df, 1000)

p1 <- ggplot(data = df, aes(x= Lab, y= Literature)) +
  geom_point() +
  facet_wrap(~Group) +
  theme_bw() +
...
p2 <- ggplot(data = df, aes(x= `Lab (new)`, y= Literature)) +
  geom_point() +
  facet_wrap(~Group) +
  theme_bw() +
  geom_smooth(method='lm', formula = "y ~ x", color = "red") +
...
p3 <- ggplot(data = df, aes(x= Lab, y= `Lab (new)`)) +
  geom_point() +
  facet_wrap(~Group)+
  theme_bw() +
  geom_smooth(method='lm', formula = "y ~ x", color = "red") +

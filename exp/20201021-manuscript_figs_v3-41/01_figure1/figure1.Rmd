...
tab$Significant[tab$Significant == "Negative"] <- "Less accessible in adult spermatogonial cells"
tab$Significant[tab$Significant == "Positive"] <- "More accessible in adult spermatogonial cells"

tab$Significant <- factor(x = tab$Significant, levels = unique(tab$Significant)[c(1, 3, 2)])

anno <- data.frame(x = c(-4, 4), y = c(5, 5), label = c("n = 760", "n = 2452"), col = c("#0072B5FF", "#BC3C29FF"))
...
ag <- gridExtra::grid.arrange(egg::set_panel_size(p = a, width = unit(7.2, "cm"), height = unit(6.62, "cm")))
...
rd$Name[rd$Name == "Adult Positive"] <- "More accessible in adult spermatogonial cells"
rd$Name[rd$Name == "Adult Negative"] <- "Less accessible in adult spermatogonial cells"
...
bg <- gridExtra::grid.arrange(egg::set_panel_size(p = b, width = unit(3.1, "cm"), height = unit(6.37, "cm")))
...
go_df$Group[go_df$Group == "Adult Positive"] <- "More accessible in adult spermatogonial cells"
go_df$Group[go_df$Group == "Adult Negative"] <- "Less accessible in adult spermatogonial cells"
...
c <- ggplot(data = arrange(go_df, Hyper_Fold_Enrichment), mapping = aes(
  ...
  facet_wrap(~Group, scales = "free_y", labeller = label_wrap_gen(width = 25), nrow = 2) +
  theme_bw(base_family = "Helvetica") +
  th +
  guides(
...
cg <- gridExtra::grid.arrange(egg::set_panel_size(p = c, width = unit(3.5, "cm"), height = unit(4.5, "cm")))
...
go_df_s$Group <- gsub(pattern = "Adult Positive", replacement = "More accessible in adult spermatogonial cells", x = go_df_s$Group)
go_df_s$Group <- gsub(pattern = "Adult Negative", replacement = "Less accessible in adult spermatogonial cells", x = go_df_s$Group)
...
dg <- gridExtra::grid.arrange(egg::set_panel_size(p = d, width = unit(3.5, "cm"), height = unit(4.5, "cm")))
...
go_df$name <- plgINS:::breakStrings(x = as.character(go_df$name), minSizeForBreak = 40)
...
c1 <- ggplot(data = arrange(go_df, Hyper_Fold_Enrichment), mapping = aes(
  x = name,
  y = Hyper_Fold_Enrichment,
  color = Hyper_Adjp_BH,
  size = Total_Genes_Annotated
...
cg1 <- gridExtra::grid.arrange(egg::set_panel_size(p = c1, width = unit(3.5, "cm"), height = unit(7.5, "cm")))
...
go_df_s$name <- plgINS:::breakStrings(x = as.character(go_df_s$name), minSizeForBreak = 40)
...
d1 <- ggplot(data = go_df_s, mapping = aes(
  x = name,
  y = Hyper_Fold_Enrichment, color = Hyper_Adjp_BH,
  size = Total_Genes_Annotated,
)) +
...
dg1 <- gridExtra::grid.arrange(egg::set_panel_size(p = d1, width = unit(3.5, "cm"), height = unit(7.5, "cm")))

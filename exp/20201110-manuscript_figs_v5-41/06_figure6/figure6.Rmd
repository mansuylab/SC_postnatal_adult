...
tf_pos$Group <- paste(tf_pos$Group, "in adult spermatogonia")
...
# tf_dea_pos$Group <- plgINS::breakStrings(x = tf_dea_pos$Group, minSizeForBreak = 10)
tf_dea_pos$Group <- gsub(pattern = "More accessible", replacement = "More accessible\n", x = tf_dea_pos$Group)
tf_dea_pos$Group <- gsub(pattern = "spermatogonia", replacement = "\nspermatogonia", x = tf_dea_pos$Group)

c <- ggplot(tf_dea_pos, aes(x = Motif, y = Group)) +
  geom_point(aes(size = -log10(q.value..Benjamini.), color = logFC), alpha = 0.7, stroke = 1.5) +
...
cg <- gridExtra::grid.arrange(egg::set_panel_size(p = c, width = unit(18.5, "cm"), height = unit(2.5, "cm")))
...
tf_neg$Group <- paste(tf_neg$Group, "in adult spermatogonia")
...
tf_dea_neg$Group <- gsub(pattern = "Less accessible", replacement = "Less accessible\n", x = tf_dea_neg$Group)
tf_dea_neg$Group <- gsub(pattern = "spermatogonia", replacement = "\nspermatogonia", x = tf_dea_neg$Group)
...
ag <- gridExtra::grid.arrange(egg::set_panel_size(p = a, width = unit(10, "cm"), height = unit(2.5, "cm")))

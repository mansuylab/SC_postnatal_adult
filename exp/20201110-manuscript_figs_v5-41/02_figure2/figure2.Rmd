...
b <- Heatmap(
  ...
  height = unit(x = 8.5, units = "cm"), width = unit(x = 4, units = "cm"),
  heatmap_legend_param = list(
    title = expression(Log[2] ~ "FC"),
    legend_height = unit(1, "cm"),
    # title_position = "leftcenter-rot",
...
tab$pathway <- gsub(pattern = "dna", replacement = "DNA", x = tab$pathway)
tab$pathway <- gsub(pattern = "rna", replacement = "RNA", x = tab$pathway)


tab$pathway <- breakStrings(tab$pathway, nb = 2, minSizeForBreak = 30)
...
c <- ggplot(tab, mapping = aes(
  ...
  scale_colour_gradientn(
    colors = myPalette(n = nrow(tab)),
    # colors = rev(cividis(n = nrow(go_df))),
    # guide = guide_colorbar(reverse = TRUE),
    breaks = c(ceiling(min(-log10(tab$padj))), floor(max(-log10(tab$padj))))
  ) +
...
cg <- gridExtra::grid.arrange(egg::set_panel_size(p = c, width = unit(3.5, "cm"), height = unit(12, "cm")))

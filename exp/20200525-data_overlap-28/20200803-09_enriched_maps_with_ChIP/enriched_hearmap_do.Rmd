...
date: "<b>Created on:</b> 2020-08-03 <br> <b>Updated on:</b> `r Sys.Date()`"
output:
...
create_heatmap <- function(split = T) {
  ...
  if (split) {
    ...
    sa <- sa[lapply(sa, nrow) > 10]
    sa1 <- sa[lapply(sa, nrow) > 500]
    
    sa2 <- sa[lapply(sa, nrow) < 500 & lapply(sa, nrow) > 100]
    sa2 <- plyr::ldply(sa2, data.frame)[,-1]
    
    sa3 <- sa[lapply(sa, nrow) < 100 & lapply(sa, nrow) > 40]
    sa3 <- plyr::ldply(sa3, data.frame)[,-1]
    
    sa4 <- sa[lapply(sa, nrow) < 40]
    sa4 <- plyr::ldply(sa4, data.frame)[,-1]
      
    sp <- sa1
    sp[["region_100_500"]] <- sa2
    sp[["region_40_100"]] <- sa3
    sp[["region_10_40"]] <- sa4

...
list_s1 <- lapply(sp_df_s1, function(x) inner_join(mat[, 1:73], x)$nearestTSS.gene_name)
list_s2 <- lapply(sp_df_s2, function(x) inner_join(mat[, 1:73], x)$nearestTSS.gene_name)

# list_s1 <- list_s1[lapply(list_s1, length) > 10]
# list_s2 <- list_s2[lapply(list_s2, length) > 10]
```

## Filtering based on number of genes
```{r}
list_s1 <- purrr::compact(
  lapply(list_s1, function(x) if (length(x) > 20) {
    return(x)
  })
)

list_s2 <- purrr::compact(
  lapply(list_s2, function(x) if (length(x) > 20) {
    return(x)
  })
)
```

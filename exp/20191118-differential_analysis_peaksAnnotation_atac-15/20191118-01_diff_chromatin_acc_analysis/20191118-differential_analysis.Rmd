...
# Library
```{r 20191118-differential-analysis-1, message=FALSE, warning=FALSE}
library(csaw)
library(edgeR)
library(dplyr)
library(ChIPseeker)
...
# Reading the counts
```{r 20191118-differential-analysis-2 }
# BAM files
...
## Function
```{r 20191118-differential-analysis-3 }
...
## Filter counts
```{r 20191118-differential-analysis-4 }
...
## TMM normalization
```{r 20191118-differential-analysis-5 }
...
design <- model.matrix(~ 0 + factor(gsub("_.*", "", colnames(y1))))
colnames(design) <- gsub(".*)", "", colnames(design))
...
```{r 20191118-differential-analysis-9 }
...
## MA plots
```{r 20191118-differential-analysis-10 }
...
```{r 20191118-differential-analysis-11 }
data@metadata$annotation <- peakAnno
offsets@metadata$annotation <- peakAnno
save(data, offsets, file = "output/atac_diff_chromatin_accessibility.RData")
```

```{r 20191118-differential-analysis-12}
tab <- list(
...
writexl::write_xlsx(
  x = tab.sig, path = "./output/atac_diff_chromatin_accessibility_lfc1_qval05.xlsx",
  col_names = T, format_headers = T
)
```
...
# SessionInfo
```{r 20191118-differential-analysis-13 }

...
dar <- data.frame(rowData(data), stringsAsFactors = F, check.names = F)
```
...
dar.map <- dar[abs(dar$`diffAccessibility-logFC`) >= 1 &
                 dar$`diffAccessibility-qvalue` <= 0.05, ]
...
# Save table
```{r}
writexl::write_xlsx(tab, "./output/dea_genes_mapped_to_diff_acc.xlsx", 

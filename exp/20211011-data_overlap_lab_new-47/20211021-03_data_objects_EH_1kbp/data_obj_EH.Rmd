---
title: "DO: Data objects for Enriched Heatmaps (1kbp)"
...
if (!file.exists(nm_rna_counts)) {
  ...
  mat_RNA_counts_merged <- data.frame(
    ...
    PNW21 = rowMeans(mat_RNA_counts[, grep(pattern = "PNW21", x = colnames(mat_RNA_counts))])
  )
...
if (!file.exists(nm_rna_counts_logFC)) {
  ...
  lfc1 <- dea.limma$`pnd15 vs pnd8`[,1:2]
  lfc2 <- dea.limma$`pnw21 vs pnd15`[,1:2]
  
  lfc <- merge(lfc1, lfc2, by = "Genes", all = T)
  colnames(lfc) <- c("Gene", "PND15vsPND8", "PNW21vsPND15")

...
draw_EH <- function(x, k, name, ht, type, log = FALSE, outdir, rm) {
  ...
  names(cnl) <- c("Category 1", "Category 2", "Category 3", "Category 4", "Category 5", "Category 6", "More acc.", "Less acc.")
  ...
  hm_an <-Heatmap(matrix = partition, 
                   col = col_an, 
                   split = par, 
                   show_row_names = F, 
                   row_title_rot = 0,
                   ...
                   column_names_gp  = gpar(fontsize = 0, fontfamily = "Helvetica"),
                   width = unit(3, "mm") )
  ...
  lgd = Legend(at = l, title = "Categories", 
               legend_gp = gpar(fill = cnl[l]), 
               labels_gp = gpar(fontsize = 6, fontfamily = "Helvetica"), 
  ...
  atac_PND15 <- make_EH(
    ...
    partition = partition, axis_name = axis_name, ht = ht, la = hm_an
  )
  ...
  if (type == "wo") {
    if (all(is.na(mat_RNA_counts[x, ]))) {
      ...
      ht_list <- hm_atac + hm_rna_counts_logFC
    }
  } else {
    if (all(is.na(mat_RNA_counts[x, ]))) {
      if (rm == "meth") {
        ht_list <- hm_atac + hm_chip
      } else {
        ht_list <- hm_atac + hm_chip + hm_bs
      }
    } else if (name == "distal" | name == "distal_accUp" | name == "distal_accDown" | name == "proximal_inactive") {
      if (rm == "meth") {
        ht_list <- hm_atac + hm_chip

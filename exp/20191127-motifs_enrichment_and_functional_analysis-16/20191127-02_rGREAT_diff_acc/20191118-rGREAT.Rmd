...
bed <- bed[grep(pattern = "chr", x = bed$seqnames),]
...
# Save tables
```{r}
writexl::write_xlsx(x = tb$`GO Molecular Function`, path = "output/GO_MF.xlsx", col_names = T, format_headers = T)
writexl::write_xlsx(x = tb$`GO Biological Process`, path = "output/GO_BP.xlsx", col_names = T, format_headers = T)
writexl::write_xlsx(x = tb$`GO Cellular Component`, path = "output/GO_CC.xlsx", col_names = T, format_headers = T)
```

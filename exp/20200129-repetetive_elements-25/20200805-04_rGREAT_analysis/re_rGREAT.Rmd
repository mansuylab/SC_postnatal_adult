...
perform_rGREAT <- function(df, FC, bg, fName, split = FALSE, sp_col = NULL, split_bg = FALSE) {
  ...
  great <- foreach(i = 1:length(sp_d)) %dopar% {
    ...
    bp <- paste0("output/subcategory/", n, "_GO_BP.xlsx")
    mf <- paste0("output/subcategory/", n, "_GO_MF.xlsx")
    cc <- paste0("output/subcategory/", n, "_GO_CC.xlsx")
...
```{r re-rGREAT-17, message=FALSE, warning=FALSE}

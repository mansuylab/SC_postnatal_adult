...
da_f <- da[da$`diffEnrichment-qvalue` <= 0.05 & abs(da$`diffEnrichment-logFC`) >= 1, ]
```
...
perform_homer <- function(df, FC, bg, out, fName,
  ...
  if (is.null(DB)) {
    for (i in 1:length(sp_d)) {
      ...
      find_motifs_genome(
        x = x[1:500, c(1:3, 11, 4:10, 12:ncol(x))],
        path = o,
        genome = "mm10",
...
```{r 2020-homer-6, warning=FALSE, message=FALSE, eval=T}
perform_homer(
...
<!-- ## Analysis Homer DB -->
<!-- ```{r 2020-homer-5, warning=FALSE, message=FALSE, eval=F} -->
<!-- perform_homer( -->
<!--   df = da_f, FC = "all", bg = bg, -->
